...
func getStatus(user string) string {
	...
	v, ok = sc.Get(time.Now().Format("2006-01") + user)
	if ok {
		monthly = v.(uint64)
	}
	v, ok = sc.Get(time.Now().Format("2006-01-02") + user)
	if ok {
		today = v.(uint64)
	}
...
func saveStatus() {
	...
	w.WriteLine(fmt.Sprintf("\nThroughput:\nSend: %s   Receive: %s\n", fmtBytes(svr.WriteCount()), fmtBytes(svr.ReadCount())))
